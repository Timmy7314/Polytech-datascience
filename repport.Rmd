...
text(fit)
fit
table(predict(fit, AWine5050[-SAWine5050,], type="vector"), AWine5050[-SAWine5050, "quality"])
...
text(fit)
fit
table(predict(fit, RWine[-SRWine,], type="vector"), RWine[-SRWine, "quality"])
```

Vins blancs
```{r}
SWWine<-c(sample(dim(WWine)[1], round(0.8 * dim(WWine)[1], digits = 0)))
fit<-rpart(WWine$quality~ ., data = WWine, method = "class", minbucket=50)
...
plot(fit)
text(fit)
table(predict(fit, WWine[-SWWine,], type="class"), WWine[-SWWine, "quality"])
```
...
```{r}
CWWine = WWine[which(WWine$quality >=5 & WWine$quality <= 7),]
SCWWine = c(sample(dim(CWWine)[1], round(0.8* dim(CWWine)[1], digits = 0)))
fit<-rpart(CWWine$quality~ ., data = CWWine, subset = SCWWine, method = "class")
plot(fit)
text(fit)
printcp(fit)
pred<-table(predict(fit, CWWine[-SCWWine,], type="class"), CWWine[-SCWWine, "quality"])
1-sum(diag(pred))/sum(pred)
pred
```